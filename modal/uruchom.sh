...
# uruchom.sh etap2 [godziny] [epoki]      - Stage 2 (Korzeniewski) w tle
...
case "${1:-stan}" in
  ...
  etap1)  $M run --detach trening_modal.py --etap 1 --godziny "${2:-6}" --epoki "${3:-6}" ;;
  etap2)  $M run --detach trening_modal.py --etap 2 --godziny "${2:-8}" --epoki "${3:-10}" ;;
